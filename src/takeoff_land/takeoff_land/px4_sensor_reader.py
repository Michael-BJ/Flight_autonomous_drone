...
class PX4SensorReader(Node):
    ...
    def get_sensor_snapshot(self) -> dict:
        ...
        # GPS *quality* — what the pre-arm gate in takeoff_land_node actually
        # reads. Kept separate from the "gps" block above because that one is
        # only position, and a position is published even when it is garbage.
        if self.gps_raw:
